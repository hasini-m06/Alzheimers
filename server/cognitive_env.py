...
# Fixed: imports from root-level modules (not from env. subpackage)
from models import (
...
# ─── Reward constants ─────────────────────────────────────────────────────────
_R = {
    "escalate_during_anomaly":     0.99,  # Best
    "medium_alert_during_anomaly": 0.70,
    "soft_alert_during_anomaly":   0.40,
    "do_nothing_during_anomaly":   0.01,  # Worst (missed critical)

    "do_nothing_no_anomaly":       0.60,  # Correct restraint
    "soft_alert_no_anomaly":       0.30,
    "medium_alert_no_anomaly":     0.20,
    "escalate_no_anomaly":         0.05,

    "spam_penalty":                -0.10, # Keeps total above 0 usually
}
...
class CogTraceEnv:
    ...
    def reset(self) -> Observation:
         """
         Resets the environment for a new Alzheimer's monitoring episode.
         Returns:
         Observation: The initial synthetic patient data.
         """
    ...
    def step(self, action: "Action | int | dict") -> Tuple[Observation, Reward, bool, StepInfo]:
        ...
        terminated = done
        truncated = False # Or your specific logic
